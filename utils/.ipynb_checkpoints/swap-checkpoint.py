"""
swap.py — SwapPipeline
Descarga eventos Swap de cualquier pool Uniswap V3 en Polygon (Alchemy),
enriquece cada transacción con metadata on-chain y agrega retornos forward
via yfinance.
"""
...
class SwapPipeline:
    """
    Pipeline completo para un pool Uniswap V3 en Polygon.
    ...
    pool_address : str
        Dirección del contrato del pool.
    ticker_yf : str
        Ticker de yfinance para el token0 (ej. "POL-USD").
    decimales_t0 : int
        Decimales del token0 (ej. 18 para WPOL).
    decimales_t1 : int
        Decimales del token1 (ej. 6 para USDC).
    nombre_t0 : str
        Nombre corto del token0 (ej. "pol"). Usado en columnas.
    nombre_t1 : str
        Nombre corto del token1 (ej. "usdc"). Usado en columnas.
    abi : list | None
        ABI del evento Swap. Por defecto usa el ABI estándar de Uniswap V3.
    zona : timezone
        Zona horaria para timestamps. Por defecto Colombia (UTC-5).
    cache_path : str | None
        Ruta del archivo parquet de cache. None desactiva el cache.
    """
    ...
    # ── Búsqueda de bloque por timestamp ──────────────────────────────────────
    ...
    def _enriquecer_evento(self, swap: dict) -> dict:
        ...
        # Dirección: Buy = el pool entrega t0 (amount0 < 0)
        #            Sell = el pool recibe t0 (amount0 > 0)
        direccion = (
    ...
    def _construir_df(self, eventos: list, verbose: bool) -> pd.DataFrame:
        ...
        # Cantidad neta de t1 (siempre >= 0)
        df[col_neto_t1] = (
            df[self.col_cantidad_t1] - df[col_gas_t1]
        ).clip(lower=0)
    ...
    def _descargar_velas(self, intervalo: str, horas_ventana: float) -> pd.DataFrame:
        ...
        # Buffer del doble de la ventana para cubrir gaps de yfinance
        # en intervalos cortos (1m, 2m, 5m).
        ...
        # Redondear inicio a medianoche UTC para que yfinance
        # no recorte el rango por hora del día.
        inicio = (ahora - timedelta(days=dias_req)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        # end un día adelante para incluir el día actual completo.
    ...
    def ejecutar(
        ...
        """
        Ejecuta el pipeline completo y devuelve self.df.

        1. Intenta cargar cache (salvo forzar_descarga=True).
        2. Si no hay cache, descarga eventos de Alchemy y construye el DataFrame.
        3. Agrega columnas de retorno forward para cada horizonte en yfinance.

        Parámetros
        ----------
        horas : float
            Ventana histórica a descargar.
        horizontes : list[str]
            Intervalos yfinance (ej. ["1m","5m","1h"]). None usa todos.
        forzar_descarga : bool
            Ignora el cache existente y descarga de nuevo.
        paso : int
            Tamaño del lote de bloques por llamada RPC.
        pausa_seg : float
            Pausa entre lotes (throttling).
        verbose : bool
            Imprime progreso.
        """
        ...
        # 3. Retornos forward
        # Calcular horas_ventana desde el timestamp más antiguo del df real,
        # no desde el parámetro horas — el cache puede tener swaps más antiguos.
    ...
    def _agregar_retornos_forward(
        ...
        for iv in horizontes:
            ...
            df[col_ganancia] = (
                df[col_retorno] * df[self.col_cantidad_t1]
            )
